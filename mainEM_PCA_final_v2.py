...
"""
# =============================================================================
Created on Thu Jun  4 13:48:53 2020
# =============================================================================
"""
...
# %% PCA reduction to 5 features

pca = decomposition.PCA(n_components=5)
pca.fit(X_noPCA )
X = pca.transform(X_noPCA )
n_observations, n_features = X.shape

# %% model running parameters
K = 2 # number of clusters, gaussien normal distributions
r = 5 # this is set to 5(PCA components) in order to calculate e step without rank reduction
...
def initialize(x=X, k = K):
    ...
    mu1 = np.random.normal(0.0,.5,(n_features,1))
    mu2 = np.random.normal(0.0,.5,(n_features,1))
...
def m_step(tau, k):
    ...
    mc = np.sum(tau, axis= 0)   # total responsibility allocated to cluster
    pi = mc/n_observations      # Fraction of total responsibity to each cluster
    
    # calculate mu
    for k_i in range(k):
        mu.append((1/mc[k_i])*np.sum(tau[:,k_i]*X.T,axis=1))
    
    # calculate weighted covariance of assigned data
    for k_i in range(k):
        diff =  X - mu[k_i].T
        cov.append((1/mc[k_i])*np.dot(tau[:,k_i]*diff.T,diff))
...
# initialize problem
mu, cov, pi = initialize(x=X, k = K)

log_likelyhood = []
convergence = 1

iter_n=0
while convergence >= 0.00005 and iter_n < 50:
...
overall_classification_error = min(n_miss/n_observations, 1-(n_miss/n_observations))
